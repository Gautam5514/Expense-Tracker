            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-white/70">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-4">
                        <div>
                            <label htmlFor="category" className="block text-sm font-medium text-[var(--ink-700)] mb-1">Category Name</label>
                            <input
                                type="text" id="category" name="category" value={formData.category}
                                onChange={handleChange}
                                placeholder="e.g., Groceries"
                                className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-[var(--accent)] focus:border-[var(--accent)] disabled:bg-slate-100/80"
                                required
                                disabled={isEditMode} // Category cannot be changed when editing
                            />
                        </div>
                        <div>
                            <label htmlFor="limit" className="block text-sm font-medium text-[var(--ink-700)] mb-1">Monthly Limit</label>
                            <div className="relative">
                                <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-[var(--ink-500)]">₹</span>
                                <input
                                    type="number" id="limit" name="limit" value={formData.limit}
                                    onChange={handleChange}
                                    placeholder="1000" min="0" step="0.01"
                                    className="w-full pl-7 border border-slate-200 rounded-xl px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-[var(--accent)] focus:border-[var(--accent)]"
                                    required
                                />
                            </div>
                        </div>
