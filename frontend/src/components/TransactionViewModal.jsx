...
const DetailItem = ({ icon, label, value }) => (
    <div className="flex items-start">
        <div className="flex-shrink-0 w-8 pt-1 text-[var(--ink-500)]">{icon}</div>
        <div>
            <p className="text-sm font-medium text-[var(--ink-500)]">{label}</p>
            <p className="text-base font-semibold text-[var(--ink-900)] break-words">{value}</p>
        </div>
    </div>
);
    ...
    return (
        <>
            <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] border border-white/70">
                    {/* Header */}
                    <div className="p-5 border-b border-slate-100 bg-slate-50 rounded-t-2xl flex justify-between items-center">
                        <h2 className="text-xl font-semibold text-[var(--ink-900)]">Transaction Details</h2>
                    ...
                    <div className="p-6 overflow-y-auto space-y-6">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-lg text-[var(--ink-500)] mt-2">
                        ...
                        {attachmentUrl && (
                            <div className="pt-4 border-t border-slate-100">
                                <p className="text-sm font-medium text-[var(--ink-500)] mb-2">Attachment</p>
                                <div className="border border-slate-200 rounded-xl p-3 flex items-center gap-4">
                                    {isImage ? (
                                        ...
                                        <div className="flex-shrink-0 p-4 bg-slate-100 rounded-lg">
                                            <FileText size={24} className="text-[var(--ink-500)]" />
                                        </div>
                                    )}
                                        ...
                                        <div className="mt-2 flex items-center gap-2">
                                            {isImage && (
                                                <button
                                                    ...
                                                    className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold text-emerald-700 bg-emerald-100 rounded-md hover:bg-emerald-200"
                                                >
                    ...
                    {/* Action Footer */}
                    <div className="bg-slate-50 p-4 flex justify-end space-x-3 rounded-b-2xl border-t border-slate-100">
                        <button onClick={onEdit} className="flex items-center gap-2 px-4 py-2 text-sm font-medium border border-slate-200 rounded-full hover:bg-white">
                            <Edit size={16} /> Edit
                        </button>
                        <button onClick={onDelete} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-red-600 bg-red-50 border border-transparent rounded-full hover:bg-red-100">
